        ...
        if (allMatchingElements.length > 0) {
          ...
          if (fam) {
            ...
            if (!loadedFontFamilies.current.has(familyKey)) {
              ...
              link.onerror = (err) => {
                // Avoid noisy unhandled rejections by swallowing and logging
                console.warn("Font stylesheet failed to load:", familyKey, err);
                // Ensure we don't think it's loaded
                loadedFontFamilies.current.delete(familyKey);
              };
